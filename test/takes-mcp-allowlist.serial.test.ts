...
describe('think op — read-only on remote callers (Lane D landed)', () => {
  test('remote save/take is forced read-only via remote_persisted_blocked flag', async () => {
    // Without usable chat credentials, runThink returns gather-only with NO_CHAT_MODEL_AVAILABLE warning.
    ...
    try {
      ...
      expect(env.warnings).toContain('NO_CHAT_MODEL_AVAILABLE');
    } finally {
